...
}`)}function Qt(s){s.code.add(S`mat3 quaternionToRotationMatrix(vec4 q) {
float x2 = q.x + q.x;
float y2 = q.y + q.y;
float z2 = q.z + q.z;
float xx = x2 * q.x;
...
}`)}class ue extends oe{constructor(){super(...arguments),this.totalGaussians=-1,this.focalLength=-1,this.minSplatRadius=-1,this.minSplatOpacity=-1,this.tanFov=Je,this.cameraDelta=pe,this.cameraPos8k=pe}}function Be(s){const e=new le;e.varyings.add("vColor","vec4"),e.varyings.add("conicOpacity","vec4"),e.varyings.add("depth","float"),e.varyings.add("gaussianCenterScreenPos","vec2"),e.varyings.add("fragScreenPos","vec2"),e.outputs.add("fragColor","vec4",0),e.vertex.uniforms.add(new fe("splatOrderTexture",a=>a.splatOrder),new fe("splatAtlasTexture",a=>a.splatAtlas),new Ke("numSplats",a=>a.totalGaussians),new Q("focalLength",a=>a.focalLength),new Q("minSplatRadius",a=>a.minSplatRadius),new Q("minSplatOpacity",a=>a.minSplatOpacity),new et("tanFov",a=>a.tanFov),new me("cameraDelta",a=>a.cameraDelta),new me("cameraPos8k",a=>a.cameraPos8k),new _e("fullWidth",({camera:a})=>a.viewport[2]),new _e("fullHeight",({camera:a})=>a.viewport[3]),new ve("proj",a=>a.camera.projectionMatrix),new ve("view",a=>a.camera.viewMatrix),new tt("cameraPosition",a=>a.camera.eye)),e.vertex.include(Yt),e.vertex.include(Qt),e.vertex.include(Xt),e.include(at,s),e.vertex.code.add(S`float ndcToPixel(float ndcCoord, float screenSize) {
return ((ndcCoord + 1.0) * screenSize - 1.0) * 0.5;
  ...
  `),e}const He=Object.freeze(Object.defineProperty({__proto__:null,GaussianSplatPassParameters:ue,build:Be},Symbol.toStringTag,{value:"Module"}));let he=class extends oe{};function Ve(){const s=new le;return s.include(Ie),s.fragment.uniforms.add(new se("colorTexture",e=>e.color),new se("splatOutputColor",e=>e.splatColor)),s.fragment.main.add(S`vec4 color = texture(colorTexture, uv);
